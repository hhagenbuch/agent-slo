...
- **`mechanics`** (no key) is the **authoritative, deterministic proof.** Sample
  ...
  CI should trust.
- **`full`** (with a key) runs the same loop against the real deployed starter
  with real evals, and ends in the exempt fix `Promoted` and the parked version
  promoted after recovery. The promotion gate is a *live* eval at a strict
  `minPassRate 0.9`: a rollback there means a real regression, not an infra
  hiccup, because the operator now defaults to
  [`agent-evals:0.2.1`](https://github.com/hhagenbuch/agent-evals/pull/30), whose
  cold-start retry rides out a warming canary's dropped connection.

Full mode is a *better slice*, not a production platform — but neither mode
leans on flake tolerance to stay green.
...
- [x] Full-mode e2e against a real model: strict gate, no flake tolerance
      (operator defaults to the eval image with the cold-start retry)
